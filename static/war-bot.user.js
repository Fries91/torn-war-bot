...
function getFriendlyMembersForTab() {
    ...
    var myUserId = String((getMe() && getMe().user_id) || '').trim();
    var myFactionId = String(
        (getMe() && getMe().faction_id) ||
        (getFaction() && (getFaction().faction_id || getFaction().id)) ||
    ...
    function getRowId(row) {
        return String(row && (row.user_id || row.member_user_id || row.id || '')).trim();
    }

    function getRowFactionId(row) {
        return String(
            (row && (
                row.faction_id ||
    ...
    addRows((state && state.war && state.war.members) || [], 'stateWarMembers');
...
function getEnemyMembersForTab() {
    ...
    markOwn(getFriendlyMembersForTab());
    ...
    function isOwnRow(row) {
        ...
        if (sideName === 'friendly' || sideName === 'friends' || sideName === 'our' || sideName === 'ours') {
            return true;
        }
    ...
    function isEnemyRow(row) {
        ...
        // If no faction id is available, only allow rows from enemy-specific sources
        return false;
    }
    ...
    function addRows(rows, source) {
        arr(rows).forEach(function (row) {
            ...
            if (source === 'enemyOnly') {
                if (!isEnemyRow(row) && enemyFactionId) return;
            } else {
                if (!isEnemyRow(row)) return;
            }
    ...
    // Only enemy-specific sources
    addRows(warEnemiesCache, 'enemyOnly');
    addRows(state && state.enemy_members, 'enemyOnly');
    addRows((state && state.war && state.war.enemy_members) || [], 'enemyOnly');
...
function renderMembersTab() {
    var members = getFriendlyMembersForTab();
    ...
    function stateLabel(stateName, member) {
        ...
        if (stateName === 'jail') {
            var jailSecs = toNum(member.jail_seconds || member.jail_time || member.status_until);
            return jailSecs > 0 ? 'Jail (' + shortTime(jailSecs) + ')' : 'Jail';
        }
        if (stateName === 'travel') {
            var travelSecs = toNum(member.travel_seconds || member.travel_time || member.status_until);
            return travelSecs > 0 ? 'Travel (' + shortTime(travelSecs) + ')' : 'Travel';
        }
    ...
    function renderMemberRow(member) {
        var name = String(member.name || member.user_name || member.member_name || 'Unknown');
        var userId = String(member.user_id || member.member_user_id || member.id || '').trim();
        var level = String(member.level || '').trim();
        var position = String(member.position || member.role || '').trim();
        var stateName = memberState(member);
        var pillClass = statePillClass(stateName);
        var pillText = stateLabel(stateName, member);

        var energyCurrent = toNum(
            member.energy_current != null ? member.energy_current :
            member.energy != null ? member.energy :
            member.energy_now
        );
        var energyMax = toNum(
            member.energy_max != null ? member.energy_max :
            member.max_energy != null ? member.max_energy :
            150
        );

        var lifeCurrent = toNum(
            member.life_current != null ? member.life_current :
            member.hp != null ? member.hp :
            member.nerve_life
        );
        var lifeMax = toNum(
            member.life_max != null ? member.life_max :
            member.max_life != null ? member.max_life :
            member.hp_max
        );

        var medCd = toNum(member.med_cd || member.medical_cd || member.medical_cooldown || 0);
        var statusCd = toNum(member.status_until || member.hospital_seconds || member.jail_seconds || member.travel_seconds || 0);

        var profileUrl = userId ? ('https://www.torn.com/profiles.php?XID=' + encodeURIComponent(userId)) : '';
        var bountyUrl = userId ? ('https://www.torn.com/bounties.php#/!p=add&userID=' + encodeURIComponent(userId)) : '';

        return '\
          <div class="warhub-card warhub-member-row" style="margin-top:6px;padding:8px;" data-medcd-base="' + esc(medCd) + '" data-statuscd-base="' + esc(statusCd) + '" data-state-name="' + esc(stateName) + '">\
            <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:8px;">\
              <div style="min-width:0;flex:1 1 auto;">\
                <div class="warhub-name" style="font-size:12px;line-height:1.15;">' + esc(name) + '</div>\
                <div class="warhub-mini" style="margin-top:4px;">ID ' + esc(userId || '--') + '</div>\
              </div>\
              <div style="display:flex;gap:4px;flex-wrap:wrap;justify-content:flex-end;">\
                <span class="' + esc(pillClass) + '" data-statuscd>' + esc(pillText) + '</span>\
                <span class="warhub-pill neutral">Lvl ' + esc(level || '--') + '</span>\
                <span class="warhub-pill neutral">' + esc(position || 'Member') + '</span>\
              </div>\
            </div>\
\
            <div style="display:flex;gap:6px;flex-wrap:wrap;margin-top:8px;">\
              <span class="warhub-pill neutral">⚡ ' + esc(fmtNum(energyCurrent)) + '/' + esc(fmtNum(energyMax || 150)) + '</span>\
              <span class="warhub-pill neutral">❤ ' + esc(fmtNum(lifeCurrent)) + '/' + esc(fmtNum(lifeMax || 0)) + '</span>\
              <span class="warhub-pill neutral">💊 <span data-medcd>' + esc(medCd > 0 ? shortTime(medCd) : 'Ready') + '</span></span>\
            </div>\
\
...
function renderEnemiesTab() {
    ...
    function enemyState(enemy) {
        ...
        var combined = [
            ...
            String(enemy.last_action_status || ''),
            String(enemy.state || '')
        ].join(' ').toLowerCase();
    ...
    function stateLabel(stateName, enemy) {
        if (stateName === 'hospital') {
            var secs = toNum(enemy.hospital_seconds || enemy.hospital_time || enemy.status_until);
            return secs > 0 ? 'Hospital (' + shortTime(secs) + ')' : 'Hospital';
        }
    ...
    var filtered = enemies.filter(function (e) {
        ...
        var uid = String(e.user_id || e.member_user_id || e.id || '').toLowerCase();
    ...
    function renderSpyDropdown(e) {
        var userId = String(e.user_id || e.member_user_id || e.id || '').trim();
        var spy = getEnemySpyById(userId);

        if (!spy) {
            return '\
              <details style="margin-top:6px;">\
                <summary class="warhub-mini" style="cursor:pointer;">Spy Report</summary>\
                <div class="warhub-mini" style="margin-top:6px;">No spy data.</div>\
              </details>';
        }

        function fmtStat(v) {
            var n = Number(v || 0);
            return isFinite(n) && n > 0 ? fmtNum(n) : '--';
        }

        var label = spy.exact ? 'Exact Spy' : (spy.predicted ? 'Predicted Spy' : 'Spy');
        var meta = [];
        if (spy.source) meta.push(spy.source);
        if (spy.age) meta.push('Age ' + spy.age);

        return '\
          <details style="margin-top:6px;">\
            <summary class="warhub-mini" style="cursor:pointer;">' + esc(label) + '</summary>\
            <div style="margin-top:6px;display:flex;gap:6px;flex-wrap:wrap;">\
              <span class="warhub-pill neutral">STR ' + esc(fmtStat(spy.strength)) + '</span>\
              <span class="warhub-pill neutral">SPD ' + esc(fmtStat(spy.speed)) + '</span>\
              <span class="warhub-pill neutral">DEX ' + esc(fmtStat(spy.dexterity)) + '</span>\
              <span class="warhub-pill neutral">DEF ' + esc(fmtStat(spy.defense)) + '</span>\
              <span class="warhub-pill neutral">TOT ' + esc(fmtStat(spy.total)) + '</span>\
            </div>\
            <div class="warhub-mini" style="margin-top:6px;">' + esc(meta.join(' • ') || 'No source') + '</div>\
          </details>';
    }
    ...
    function renderEnemyRow(e) {
        ...
        var position = String(e.position || e.role || '').trim();
        var stateName = enemyState(e);
        var pillClass = statePillClass(stateName);
...
\
              <div style="display:flex;align-items:center;gap:4px;flex:0 0 auto;font-size:11px;white-space:nowrap;">\
                <span class="' + esc(pillClass) + '">' + esc(pillText) + '</span>\
                <span class="warhub-pill neutral">Lvl ' + esc(level || '--') + '</span>\
                <span class="warhub-pill neutral">' + esc(position || 'Member') + '</span>\
              </div>\
\
              <div class="warhub-actions" style="display:flex;align-items:center;gap:4px;flex:0 0 auto;margin-left:2px;">\
                ' + (profileUrl ? '<a class="warhub-btn" style="padding:4px 7px;font-size:11px;min-height:auto;" href="' + esc(profileUrl) + '" target="_blank" rel="noopener noreferrer">Profile</a>' : '') + '\
                ' + (bountyUrl ? '<a class="warhub-btn" style="padding:4px 7px;font-size:11px;min-height:auto;" href="' + esc(bountyUrl) + '" target="_blank" rel="noopener noreferrer">Bounty</a>' : '') + '\
              </div>\
            </div>\
          </div>';
    }
...
if (saveKeysBtn && !saveKeysBtn.__warhubBound) {
    ...
    overlay.querySelectorAll('[data-member-bounty="1"]').forEach(function (btn) {
        ...
        btn.addEventListener('click', function () {
            ...
            if (userId) {
                window.open(
                    'https://www.torn.com/bounties.php#/!p=add&userID=' + encodeURIComponent(userId),
                    '_blank',
                    'noopener,noreferrer'
                );
            }
