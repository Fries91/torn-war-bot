...
(function () {
    ...
    function targetItemId(t) {
        return String((t && (t.user_id || t.target_user_id || t.id || t.player_id)) || '').trim();
    }

    function normalizeTargetItem(t) {
        if (!t || typeof t !== 'object') return null;
        var id = targetItemId(t);
        if (!id) return null;
        return {
            user_id: id,
            name: String(t.name || t.target_name || t.player_name || 'Target').trim() || 'Target',
    ...
    function mergeTargets(primary, secondary) {
        ...
        arr(primary).concat(arr(secondary)).forEach(function (item) {
            var norm = normalizeTargetItem(item);
            if (!norm) return;
            var id = norm.user_id;
            if (seen[id]) {
                if (!seen[id].note && norm.note) seen[id].note = norm.note;
                if ((!seen[id].name || seen[id].name === 'Target') && norm.name) seen[id].name = norm.name;
                return;
            }
            seen[id] = norm;
            out.push(norm);
...
function renderTermsTab() {
    ...
    function renderFactionTab() {
        var license = (state && state.license) || {};
        var members = arr(currentFactionMembers || factionMembersCache || (state && state.members) || (state && state.faction && (state.faction.members || state.faction.items || state.faction.member_list)) || [])
        ...
        if (!members.length && canManageFaction() && !factionHydratePending) {
            factionHydratePending = true;
            setTimeout(function () {
                loadFactionMembers(true).then(function () {
                    return refreshFactionPaymentData();
                }).then(function () {
                    factionHydratePending = false;
                    renderBody();
                }).catch(function () {
                    factionHydratePending = false;
                });
