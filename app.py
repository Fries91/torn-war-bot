...
def _build_member_bar_payload(member: Dict[str, Any], api_key: str = "") -> Dict[str, Any]:
    ...
    med_cd = 0
    booster_cd = 0
    debug = {"step": "no_api_key"} if not api_key else {"step": "not_loaded"}

    if api_key:
        live = member_live_bars(api_key, user_id="")
        debug = {
            **(live.get("debug") or {}),
            "ok": bool(live.get("ok")),
...
def api_faction_members():
    ...
    viewer_row = next((x for x in items if str(x.get("user_id") or "") == viewer_user_id), {})
    return ok(
        faction_id=faction_id,
        faction_name=faction_name,
        items=items,
